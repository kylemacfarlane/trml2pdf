        ...
        for node in style_node.childNodes:
            if node.nodeType==node.ELEMENT_NODE:
                ...
                if node.localName=='blockValign':
                    ...
                    styles.append(('ALIGN', start, stop, str(node.getAttribute('value'))))
...
class _rml_flowable(object):
    ...
    def _get_cell_style(self, x, y, style, option):
        ...
        return tuple([_cmds[style], (x, y), (x, y), option])
    ...
    def _table(self, node):
        ...
        for tr in _child_get(node,'tr'):
            ...
            for td in _child_get(tr, 'td'):
                ...
                data2.append( flow )
                for tag in ('align','vAlign'):
                    if td.hasAttribute(tag):
                        print self._get_cell_style(x, y , tag, td.getAttribute(tag))
                x += 1
            if len(data2)>length:
                length=len(data2)
                for ab in data:
